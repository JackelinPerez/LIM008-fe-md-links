...
exports.getAllMds = exports.filtroDirAndFile = void 0;

const filtroDirAndFile = (document, extFile, dir, fs) => {
  const objMdAndDir = {
    file: [],
    dir: []
  };
  return document.reduce((anterior, doc) => {
    ...
    if (validateDir) objMdAndDir.dir.push(rutaAllDoc);else if (doc.toLowerCase().indexOf(extFile) !== -1) objMdAndDir.file.push(rutaAllDoc);
    return objMdAndDir;
  }, {});
};

exports.filtroDirAndFile = filtroDirAndFile;

const getAllMds = (dir, exten, contAllFile, contMain, lengthMain) => {
  let auxObjMdAndDir = {};
  ...
  if (validateDir) {
    ...
    if (items.length > 0) {
      ...
      if (auxObjMdAndDir.file.length > 0) auxObjMdAndDir.file.forEach(fileMD => contAllFile.push(fileMD));
